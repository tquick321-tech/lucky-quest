...
router.post('/add', authenticateToken, [
  ...
  body('type').isIn(['game_reward', 'challenge_reward', 'referral_bonus', 'daily_bonus', 'wheel_reward', 'chest_reward', 'season_pass_reward', 'admin_grant']),
], async (req, res) => {
...
router.post('/withdraw', authenticateToken, [
  ...
  body('method').isIn(['paypal', 'bank_transfer', 'crypto', 'gift_card']),
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
